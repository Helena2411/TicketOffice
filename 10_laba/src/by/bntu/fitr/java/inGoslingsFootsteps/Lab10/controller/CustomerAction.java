...
public class CustomerAction {
    ...
    public static  void choicebyCustomer(FlightController flightController, Customer customer, int action) throws IOException {
        try {
            if (action == 1) {
                Flight[] availableFlight = flightController.GetAvailableFlight(customer);
                Printer.printArray(availableFlight);
                int index = UserInput.inputInt("Choose flight");
                flightController.bookTicket(availableFlight[index - 1], customer);
                LOG.info("Ticket is booked");
            }
            else if (action == 2) {
                Printer.printArray(flightController.GetFlightsBookedByCustomer(customer));
                int index = UserInput.inputInt("Choose flight");
                flightController.returnTicket(flightController.GetFlightsBookedByCustomer(customer)[index - 1], customer);
